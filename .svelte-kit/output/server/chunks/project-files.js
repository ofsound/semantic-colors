...
function parseCandidateValues(rawValue) {
	...
	if (lightDark.light || lightDark.dark) return {
...
function suggestToken(sourceName) {
	...
	for (const entry of NAME_MAP) {
		...
		if (!best || entry.confidence > best.confidence) best = {
			sourceName,
			rawValue: "",
...
async function readSession(cwd) {
	const raw = await safeReadText(path.join(cwd, SESSION_PATH));
	if (!raw) return { configPath: defaultConfigPath(cwd) };
	try {
		return { configPath: JSON.parse(raw).configPath || defaultConfigPath(cwd) };
	} catch {
		return { configPath: defaultConfigPath(cwd) };
	}
}
async function writeSession(cwd, configPath) {
	...
	await writeFile(sessionFilePath, JSON.stringify({ configPath }, null, 2));
}
